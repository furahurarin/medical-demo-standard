...
export const metadata: Metadata = {
  title: "医師紹介｜架空クリニック",
  description:
    "院長のご挨拶、専門分野、資格・所属、診療方針、対応領域、初診の流れをご案内します。写真は院長のみ掲載。",
...
type Director = {
  ...
  specialties?: string[];
  credentials?: string[];
  careers?: { period: string; text: string }[];
  photoSrc: string;
  photoAlt: string;
};
...
const DIRECTOR: Director = {
  ...
  dept: "内科",
  intro:
    "当院は、院長が初診からフォローまで一貫して担当する小規模クリニックです。必要十分な検査と丁寧な説明を重視し、生活背景に合わせた無理のない治療計画をご提案します。都心の通いやすさと、顔の見える診療体験の両立を目指しています。",
  specialties: [
    "総合内科",
    "生活習慣病（高血圧・脂質異常症・糖尿病）",
    "かぜ・胃腸炎などの一般内科",
  ...
  careers: [
    { period: "20XX年", text: "国立◯◯大学 医学部卒業" },
    { period: "20XX年", text: "◯◯総合病院 内科 勤務（急性期・総合診療を担当）" },
    { period: "20XX年", text: "都内クリニック 副院長" },
    { period: "20XX年〜", text: "架空クリニック 院長" },
  ],
...
export default function StaffPage() {
  return (
    <main className="min-h-screen">
      {/* このページにいる間だけ、同ページ内ジャンプリンクを非表示にする */}
      <style>{`
        a[href^="#"], a[href^="/staff#"] { display: none !important; }
      `}</style>
      ...
      <section className="border-b border-gray-100 bg-white">
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
          ...
          <p className="mt-3 text-gray-600">
            小規模体制により、院長が継続的にお顔と状態を把握したうえで診療します。
          </p>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16">
        <div className="mx-auto grid max-w-6xl grid-cols-1 items-start gap-8 px-4 sm:grid-cols-12 sm:px-6">
          ...
          <div className="sm:col-span-7">
            ...
            {/* 専門・資格 */}
            {(DIRECTOR.specialties?.length || DIRECTOR.credentials?.length) && (
              <div className="mt-6 grid gap-6 sm:grid-cols-2">
                {DIRECTOR.specialties?.length ? (
                  <div>
            ...
            {/* 経歴（period重複に備え index 併用） */}
            {DIRECTOR.careers?.length ? (
              <div className="mt-6">
                <h3 className="text-sm font-semibold text-gray-900">経歴</h3>
                <ul className="mt-2 space-y-1 text-sm text-gray-700">
                  {DIRECTOR.careers.map((c, i) => (
                    <li key={`${i}-${c.period}`}>
                      <span className="inline-block w-20 text-gray-500">{c.period}</span>
                      <span>{c.text}</span>
                    </li>
                  ))}
                </ul>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <ol className="mt-4 grid gap-3 sm:grid-cols-2">
            {[
              {
                title: "ご来院・受付",
                text: "健康保険証・各種医療証、お薬手帳をご持参ください。",
              },
              {
                title: "問診",
                text: "症状の経過やご不安な点を丁寧に伺います。必要に応じて検査をご提案します。",
              },
              {
                title: "診察・検査",
                text: "院長が診察を行い、過不足のない検査を選択します。",
              },
              {
                title: "結果説明・お会計",
                text: "結果と治療方針をご説明し、次回の受診目安をご案内します。",
              },
            ].map((step, i) => (
              <li key={`${i}-${step.title}`} className="rounded-xl bg-white p-4 shadow-sm">
